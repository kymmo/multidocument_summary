...
# --- Global Models (for subprocess initialization) ---
     ...
     try:
          ...
          for original_training_idx, sample_items in samples_in_batch.items():
               ...
               for item_info in sample_items:
                    ...
                    # 1. Similarity Edges
                    similarity_edges = compute_edges_similarity(doc_sent_texts, edge_similarity_threshold)
...
def define_node_edge_opt_parallel(documents_list, edge_similarity_threshold=0.6):
     ...
     print(f"Task preparation took {prepare_time:.4f} s for {len(tasks)} task batches.")

     cpu_num = auto_workers()
     print(f"Using {cpu_num} worker processes.")
     ...
     print(f"Parallel processing took {processing_time:.4f} s.")
