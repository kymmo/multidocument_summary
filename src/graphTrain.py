...
from models.HetSumGraph import HSumGraph
from graphTester import SLTester, TestPipLine
from models.dataloader import ExampleSet, graph_collate_fn
from models.embedding import Word_Embedding
from models.vocabulary import Vocab
...
def run_training(model, train_loader, valid_loader, valset, hps, train_dir):
     ...
     for epoch in range(1, hps.n_epochs + 1):
          ...
          for i, (G, index) in enumerate(train_loader):
               ...
               outputs = model.forward(G)  # [n_snodes, hidden_size]
...
def run_eval(model, loader, valset, hps, best_loss, best_F, non_descent_cnt, saveNo):
     ...
     with torch.no_grad():
          tester = TestPipLine(model, hps.m)
     ...
     # tester.getMetric()
     F = 0 # tester.labelMetric # No label metric used.
     ...
     if best_F is None or best_F < F: # always not met.
